export type FactoryId = 'lhg' | 'lyv' | 'lvl'
...
let _factoryId: FactoryId = 'lhg'

export const getFactoryId = (): FactoryId => _factoryId

export const setFactoryId = (id: string): void => {
  if (id === 'lhg' || id === 'lyv' || id === 'lvl') {
    _factoryId = id
  }
}
...
export const setApiBase = (_base: string): void => {
  /* no-op: routing is now server-side */
}